<template>
	<div id="q-app" :class="$color('textMain')" class="q-electron-drag">
		...
		<fullscreen-modal v-if="$store.getters.currentTaskIdForModal || $store.getters.showCreateTaskModal" close-on-bg-click @close="$store.dispatch('closeTaskModal')">
			<template #modal-body>
				<task-form :is-modal="true" :modal-task-id="$store.getters.currentTaskIdForModal" :modal-project-category-id="$store.getters.createTaskInProjectCategoryId" :status-id="$store.getters.createTaskInStatusId" @close="$store.dispatch('closeTaskModal')"></task-form>
			</template>
		</fullscreen-modal>
	</div>
</template>

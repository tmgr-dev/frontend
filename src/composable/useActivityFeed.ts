...
export function useActivityFeed(
	workspaceId?: number | { value: number },
	...
	const canLoadMore = computed(() => {
		return (
			hasMore.value && !loading.value && !loadingMore.value && !refreshing.value
		);
	});
	...
	const buildApiParams = (page = 1): ActivityFeedParams => ({
		...
		filters: {
			...
			user_id:
				filters.value.user_id === 'all' ? undefined : filters.value.user_id,
		},

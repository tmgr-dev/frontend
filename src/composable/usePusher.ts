...
export function usePusher(): UsePusherReturn {
	...
	const createPusherConfig = (): PusherConfig => {
		...
		const port = parseInt(
			(import.meta as any).env.VITE_PUSHER_PORT || '6001',
			10,
		);
		const authEndpoint =
			(import.meta as any).env.VITE_API_BASE_URL + 'broadcasting/auth';
		...
		return {
			...
			channelAuthorization: {
				...
				transport: 'ajax',
				customHandler: createChannelAuthorizer({
					authEndpoint,
			...
			connectionError.value = {
				message:
					error instanceof Error
						? error.message
						: 'Failed to initialize Pusher connection',
				type: 'network',
	...
	const subscribe = (channelName: string, events: EventHandlers): string => {
		...
		try {
			...
			channel.listen(
				'.dashboard.updated',
				(data: { statistics: Partial<DashboardStatistics> }) => {
					const sub = subscriptions.get(channelName);
					if (sub) {
						sub.handlers.forEach((h) =>
							h.onDashboardUpdated?.(data.statistics),
						);
					}
				},
			);
			...
			channel.listen(
				...
				(data: CommentReactionsUpdatedEvent) => {
					const sub = subscriptions.get(channelName);
					if (sub) {
	...
	const unsubscribeHandlerFromWorkspace = (
		workspaceId: number,
		subscriptionId: string,
	): void => {
		const channelName = `App.Workspace.${workspaceId}`;

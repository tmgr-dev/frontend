...
export function handleDashboardStatistics(
	response: unknown,
...
export function createLoadingStateManager() {
	...
	const setSuccess = (key: string): DetailedLoadingState => {
		...
		const state: DetailedLoadingState = {
			...
			error: null,
			endTime: Date.now(),
			duration: currentState.startTime
	...
	async getStatistics(
		...
		try {
			const response = await fetch(
				`${this.baseUrl}/workspaces/${workspaceId}/dashboard/statistics`,
			);
	...
	async getActivities(
		...
		try {
			const params = new URLSearchParams({
				...
				...(filters?.user_id &&
					filters.user_id !== 'all' && { user_id: filters.user_id.toString() }),
		...
		try {
			const params = new URLSearchParams({
				...
				...(filters?.is_overdue !== undefined && {
					is_overdue: filters.is_overdue.toString(),
				}),
				...(filters?.has_timer !== undefined && {
					has_timer: filters.has_timer.toString(),
				}),
				...(filters?.search && { search: filters.search }),
			});
...
export async function exampleUsage() {
	...
	const tasksResult = await apiClient.getRecentTasks(
		workspaceId,
		{
			priority: 'high',
			is_overdue: true,
...
export async function exampleDashboardActionsUsage() {
	...
	const refreshResult = await refreshDashboardSection(
		workspaceId,
		'statistics',
		{
			force: true,

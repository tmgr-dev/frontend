	...
	<BaseLayout no-copyright>
		<template #body>
			...
			<div class="fixed bottom-8 right-8 z-50">
				<div
					ref="trashBinRef"
					class="trash-bin w-12 h-12 text-gray-600 dark:text-gray-400"
				>
					<!-- Trash Bin SVG -->
					<svg
			...
			<div class="relative">
				...
				>
					<li
						...
						class="task-item m-2 rounded-xl border py-3.5 px-5 dark:border-0 dark:bg-gray-800 relative"
						:style="{ backgroundColor: task.isAnimating ? 'transparent' : undefined }"
					>
						<!-- Task Content -->
						<div class="task-content flex items-center justify-between w-full"
								 :class="{ 'opacity-0': task.isAnimating }">
							<!-- Left Side: Checkbox and Title -->
							<div class="flex items-center">
								<button
									@click="completeTask(task)"
									class="mr-3 rounded-full p-1 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
                  ...
                  {{ task.title }}
							...
							<div class="flex items-center">
								<button
									...
									class="mr-2 rounded-full p-1.5 text-gray-500 hover:bg-gray-200 hover:text-blue-500 dark:hover:bg-gray-700 transition-colors"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										width="18"
								...
								<button
									...
									class="rounded-full p-1.5 text-gray-500 hover:bg-gray-200 hover:text-red-500 dark:hover:bg-gray-700 transition-colors"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										width="18"
						...
						<div
							...
							class="paper-animation absolute top-0 left-0 w-full h-full z-max"
						>
							<svg class="crumpled-paper" width="200px" height="200px" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
								<path
									fill-rule="evenodd"
									clip-rule="evenodd"
...
import {
	...
	deleteDailyTask,
	completeDailyTask,
	archiveDailyTask, getArchivedDailyTasksCount
} from 'src/actions/tmgr/daily-tasks';
...
function createShrimpAnimation(taskEl: HTMLElement, onComplete: () => void) {
	...
	tl.to(paperAnimation, {
		...
		.to(paperAnimation, {
			...
			motionPath: {
				path: [
					{ x: windowCenter - startX - taskRect.width / 2, y: centerY - startY },
...
async function deleteTask(task: ExtendedTask) {
	try {
		...
		const taskIndex = tasks.value.findIndex(t => t.id === task.id);
		if (taskIndex !== -1) {
			tasks.value[taskIndex] = { ...tasks.value[taskIndex], isAnimating: true };
		}

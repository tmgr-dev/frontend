<script setup lang="ts">
	...
	async function updateSettings() {
		try {
			...
			const preferredEditorSetting = settings.value.find(
				(setting: any) => setting.key === 'preferred_editor',
			);
			...
			if (preferredEditorSetting) {
				...
				if (
					editorValue &&
					(editorValue === 'block' || editorValue === 'markdown')
				) {
					...
					if (store.state.user && store.state.user.settings) {
						...
						store.state.user.settings = store.state.user.settings.map(
							(setting: any) => {
			...
			if (preferredEditorSetting) {
				...
				if (
					normalizedNewValue &&
					normalizedNewValue !== normalizedOriginalValue
				) {
					toaster.toast({
						variant: 'default',
					...
					if (
						normalizedNewValue === 'block' ||
						normalizedNewValue === 'markdown'
					) {
	...
	function canRemoveMember(member: WorkspaceMember) {
		...
		return (
			workspace?.user_id === store.state.user.id &&
			member.id !== workspace?.user_id
		);
	}
						...
						>
							<DialogHeader>
								...
								<DialogDescription class="sr-only"
									>Enter a name for your new workspace</DialogDescription
								>
							</DialogHeader>
					...
					<Dialog v-model:open="isOpenInvitation">
						...
						>
							<DialogHeader>
								...
								<DialogDescription class="sr-only"
									>Enter email addresses to invite members to this
					...
					>
						...
						<AlertDialogContent>
							<AlertDialogHeader>
								...
								<AlertDialogDescription>
									This action cannot be undone. This will delete your workspace.
								</AlertDialogDescription>
							</AlertDialogHeader>
							<AlertDialogFooter>
					...
					<AlertDialog v-if="!isLoading && !isOwnedActiveWorkspace">
						...
						<AlertDialogContent>
							<AlertDialogHeader>
								<AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
								<AlertDialogDescription>
									This action cannot be undone by yourself. If you wanna come
									back to this workspace ask a member of this workspace to send
									you an invitation again.
								</AlertDialogDescription>
							</AlertDialogHeader>
							<AlertDialogFooter>
								<AlertDialogCancel>Cancel</AlertDialogCancel>

<script setup lang="ts">
	...
	const getCurrentWorkspaceId = (): number => {
		try {
			...
			const routeWorkspaceId =
				route.params.workspaceId || route.query.workspaceId;
		...
		(newSettings) => {
			if (newSettings) {
				...
				if (newWorkspaceId !== workspaceId.value) {
					console.log(
						`Dashboard: Workspace changed from ${workspaceId.value} to ${newWorkspaceId}`,
					);
	...
	const workspaceUsersList = computed(() => {
		return (
			teamActivity.value?.members?.map((member) => ({
				id: member.id,
				name: member.name,
			})) || []
		);
	...
	const showActivityNotification = (activity: Activity) => {
		...
		const notification = {
			id: Date.now(),
			type: 'activity',
			message: `New ${activity.type.replace('_', ' ')}: ${
	...
	const handleMemberClick = (member: TeamMemberStatus) => {
		router.push({
			...
			params: {
				workspace_code: route.params.workspace_code,
				user_id: member.id,
			},
		});
	};
	...
	const optimisticallyUpdateStatistics = (
		updates: Partial<DashboardStatistics>,
	) => {
		...
		if (event.ctrlKey || event.metaKey) {
			switch (event.key) {
				...
				case '1':
					...
					document
						.querySelector('[aria-labelledby="statistics-heading"]')
						?.focus();
					break;
				case '2':
					event.preventDefault();
					...
					document
						.querySelector('[aria-labelledby="heatmap-heading"]')
						?.focus();
					break;
				case '3':
					event.preventDefault();
					...
					document
						.querySelector('[aria-labelledby="activity-heading"]')
						?.focus();
					break;
				case '4':
					event.preventDefault();
					...
					document
						.querySelector('[aria-labelledby="team-activity-heading"]')
						?.focus();
					break;
			}
		}
	...
	const handleDashboardUpdateAccessible = (
		updates: Partial<DashboardStatistics>,
	) => {
		...
		if (updates.completed_today) {
			announceUpdate(
				`Tasks completed today updated to ${updates.completed_today}`,
			);
		}
	};
						...
						>
									...
									<div class="flex-1">
										...
										<div v-if="canRetry" class="mt-3 flex gap-2">
											...
											<Button
												...
												class="border-yellow-300 text-yellow-700 hover:bg-yellow-50 dark:border-yellow-600 dark:text-yellow-300 dark:hover:bg-yellow-900/30"
											>
												Check Connection
											</Button>
										</div>
							...
							>
								<div class="mb-4">
									...
									>
										Your contribution activity over the past year. Each square
										represents a day, with darker colors indicating more
										activity.
									</p>
								</div>
								...
								<div
									...
									:aria-label="
										heatmapData
											? `Activity heatmap showing ${heatmapData.total_contributions} total contributions`
											: 'Activity heatmap loading'
									"
									:aria-describedby="
										heatmapData ? 'heatmap-summary' : 'heatmap-loading'
									"
								>
									<HeatmapSkeleton v-if="loadingStates.heatmap.isLoading" />

									<div
										v-else-if="loadingStates.heatmap.error"
									...
									<div v-if="heatmapData" id="heatmap-summary" class="sr-only">
							...
							>
								...
								<StatisticsGrid
									...
									:aria-describedby="
										loadingStates.statistics.isLoading
											? 'statistics-loading'
								...
								<div id="statistics-description" class="sr-only">
									Grid of workspace statistics including task counts, time
									tracking, and team metrics. Use arrow keys to navigate between
									cards.
								</div>
							</section>
							...
							<div class="dashboard-grid" role="main">
								...
								>
									...
									<section
										...
										role="region"
									>
										<h3 id="team-activity-heading" class="sr-only">
											Team Activity
										</h3>
										<TeamActivityWidget
											...
											:aria-describedby="
												loadingStates.teamActivity.isLoading
													? 'team-loading'
													: 'team-description'
											"
										/>
										...
										<div id="team-description" class="sr-only">
											Team member activity and status. Shows who is online and
											their current tasks.
										</div>
								...
								<section
									...
									role="region"
								>
									<h2 id="activity-heading" class="sr-only">
									...
									<div id="activity-description" class="sr-only">
										List of recent workspace activities. Use arrow keys to
										navigate, Enter to open activity details.
									</div>
...
<style scoped>
	...
	.heatmap-container {
		@apply min-h-48 rounded-card border border-line bg-surface p-6 shadow-tmgr-xs;
	}
	...
	.dashboard-main-content {
		@apply min-h-96 w-full;
	}

	.dashboard-sidebar {
		@apply flex min-h-64 w-full flex-col space-y-6;
	}
	...
	.loading-skeleton {
		@apply animate-pulse rounded bg-gradient-to-r from-gray-200 via-gray-300 to-gray-200 dark:from-gray-700 dark:via-gray-600 dark:to-gray-700;
	...
	@media (prefers-contrast: high) {
		...
		.dashboard-section {
			@apply rounded-lg border-2 border-gray-900 bg-white p-4 text-black dark:border-gray-100 dark:bg-black dark:text-white;
		}
	}

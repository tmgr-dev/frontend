<template>
  <div class="mx-auto w-full max-w-6xl space-y-4 p-4">
    <nav class="text-xs text-ink-subtle" aria-label="Breadcrumb">
      <router-link :to="dashboardLink" class="hover:text-ink">Dashboard</router-link>
      <span> / </span>
      <span class="text-ink">{{ stats?.name ?? 'Member' }}</span>
    </nav>

    <p v-if="notFound" class="rounded-card border border-line bg-surface p-6 text-sm text-ink-subtle">
      This user is not a member of this workspace.
    </p>

    <div v-else-if="error" class="rounded-card border border-line bg-surface p-6">
      <p class="text-sm text-ink">{{ error }}</p>
      <button type="button" class="mt-3 rounded bg-brand px-3 py-1.5 text-sm text-white" @click="refresh">
        Try again
      </button>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const { currentWorkspaceId, currentWorkspaceCode } = useCurrentWorkspace();

    const userId = computed(() => Number(route.params.user_id));
    const workspaceId = computed(() => currentWorkspaceId.value);

    const page = useMemberPage(() => currentWorkspaceId.value, () => userId.value);

    const activities = ref<Activity[]>([]);
    const activitiesLoading = ref(false);
    const activitiesPage = ref(1);
    const activitiesTotal = ref(0);
    const activitiesHasMore = computed(() => activities.value.length < activitiesTotal.value);

    const dashboardLink = computed(() => ({
      path: `/${currentWorkspaceCode.value}/dashboard`
    }));

    const openTask = (taskId: number) => store.commit('setCurrentTaskIdForModal', taskId);
    ...
    async function loadActivities(nextPage = 1) {
      ...
      if (result.success && result.data) {
        const rows = (result.data as { data?: { data?: Activity[]; meta?: { total?: number } } }).data;
        activities.value = nextPage === 1
          ? rows?.data ?? []
    ...
    onMounted(refresh);
    watch(() => [workspaceId.value, userId.value], () => refresh());

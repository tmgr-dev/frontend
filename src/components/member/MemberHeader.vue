	...
	>
			...
			<div class="flex items-start gap-3">
				...
				<div>
					...
					<p class="text-sm text-ink-subtle">
						<span
							class="rounded bg-surface-sunken px-1.5 py-0.5 text-xs uppercase"
							>{{ member?.role ?? '' }}</span
						>
		...
		<div v-if="bars.length" class="mt-5">
			...
			<div class="flex h-6 items-end gap-0.5">
				<span
					...
					:key="index"
					class="bg-brand/60 w-full rounded-sm"
		...
		>
			<span class="text-sm text-ink"
				>Working on: {{ member.current_task.title }}</span
			>
...
<script setup lang="ts">
	...
	const bars = computed(() =>
		sparklineBars(props.member?.daily_activity ?? []),
	);
	...
	const tiles = computed(() => {
		...
		const share = percentOfTeam(
			m.tracked_seconds,
			m.team?.tracked_seconds ?? 0,
		);

		return [
			{
				label: 'Tracked',
				value: formatTrackedSeconds(m.tracked_seconds),
				hint: share ? `${share} of team` : null,
			},
			{
				label: 'Done',
				value: m.done_count,
				hint: `team ${m.team?.done_count ?? 0}`,
			},
			{
				label: 'Active tasks',
			...
			},
			{
				label: 'Streak',
				value: `${m.streak}d`,
				hint: `longest ${m.longest_streak}d`,
			},
			{
				label: 'Activity',
				value: m.activity_count,
				hint: `${m.active_days} active days`,
			},
		];
	});
</script>

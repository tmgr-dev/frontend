<template>
  <section class="rounded-card border border-line bg-surface p-6 shadow-tmgr-xs">
    <div class="mb-4 flex flex-wrap gap-1" role="tablist" aria-label="Member tasks">
      <button
    ...
    <p v-if="loading && !tasks.length" class="py-6 text-center text-sm text-ink-subtle">Loading…</p>
    <p v-else-if="!tasks.length" class="py-6 text-center text-sm text-ink-subtle">Nothing here for this window.</p>
          ...
          <span class="min-w-0">
            <span class="flex items-center gap-2">
              <span
                ...
                :style="{ backgroundColor: task.status?.color || 'var(--color-line)' }"
              />
            ...
            <span class="mt-0.5 block text-xs text-ink-subtle">
              ...
              <span v-if="task.my_comments"> · {{ task.my_comments }} comments</span>
            </span>
          </span>

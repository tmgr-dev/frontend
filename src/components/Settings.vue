	...
	<BaseLayout>
		...
		<template #body>
			<div :class="`w-full md:w-auto md:flex-grow md:flex md:items-center md:block block ${$color('textMain')}`">
				<Button v-if="!pusherBeamsUserId" :color="pusherBeamsUserId ? `red` : `green`" @click="togglePushes">Web
					Pushes
				</Button>
				<Button color="blue" @click="testWebPushNotifications">Test web push notifications</Button>
				<input-field v-model="userSettings.showTooltips" placeholder="Show tooltips" type="checkbox"></input-field>
				<div>

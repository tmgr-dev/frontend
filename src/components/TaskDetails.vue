<template>
	<div class="h-full w-full overflow-y-auto">
		...
		<div class="flex items-center justify-between">
			...
			<button
				@click="$emit('close')"
				class="cursor-pointer rounded-full p-2 hover:bg-gray-100 dark:hover:bg-gray-800"
			>
				<svg
		...
		<div class="space-y-6">
			...
			<section class="space-y-4">
				<div>
					<TextField
						...
						class="mt-1 w-full rounded-md py-2 dark:border dark:border-gray-300 dark:border-gray-600 dark:bg-gray-800"
				...
				<div>
					<textarea
						...
						placeholder="Enter your routine's description here."
						rows="4"
						class="mt-1 w-full rounded-md border border-gray-300 bg-white px-3 py-2 dark:border-gray-600 dark:bg-gray-800"
					/>
				</div>
			</section>

			<section class="flex items-center justify-between space-x-4">
				<span @click="taskData.is_recurring = !taskData.is_recurring"
					>Recurrence</span
				>
				<Switch
					:checked="taskData.is_recurring"
					@update:checked="taskData.is_recurring = !taskData.is_recurring"
				/>
			</section>
			...
			<section v-else class="space-y-4 rounded-lg border dark:border-gray-700">
				<div class="">
					...
					>
					<div
						class="mt-1 flex items-center space-x-4"
						v-if="taskData.recurrence.time"
					>
						<select
				...
				<div v-if="true" class="space-y-4">
					...
					<!-- Weekly Options -->
					<div
						v-if="taskData.recurrence.frequency === 'WEEKLY'"
						class="space-y-2"
					>
						<label
							class="block text-sm font-medium text-gray-700 dark:text-gray-300"
			...
			<div class="flex justify-end gap-2 pb-4">
				<Button
					class="w-full rounded-xl p-2 dark:bg-gray-800 dark:hover:bg-gray-500 dark:focus:bg-gray-600"
					@click="saveChanges"
					>Save Changes</Button
				>
				<Button
					class="rounded-xl p-2 dark:bg-gray-800 dark:hover:bg-gray-500 dark:focus:bg-gray-600"
					@click="completeTask"
				>
...
<script setup lang="ts">
	...
	const taskData = ref<ExtendedTask>({
		...
		scheduled_date:
			props.task.scheduled_date || new Date().toISOString().split('T')[0],
	...
	async function completeTask() {
		...
		if (taskData.value.instance_id) {
			await completeDailyTaskInstance(
				taskData.value.id,
				taskData.value.instance_id,
			);
		} else {

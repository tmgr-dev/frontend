<template>
	<div>
		...
		<div v-else class="space-y-4">
			...
			<div
				v-if="!agents.length"
				class="rounded-card border border-line bg-surface-sunken p-8 text-center"
			>
				<svg
					class="mx-auto mb-3 h-12 w-12 text-ink-faint"
					fill="none"
					stroke="currentColor"
					viewBox="0 0 24 24"
				>
				...
				<p class="text-ink-subtle">No agents launched yet</p>
			</div>
			...
			<div v-else class="space-y-3">
				...
				>
					...
					<div v-if="agent.branch_name" class="mb-2 text-sm text-ink">
						...
						<code
							class="ml-1 rounded border border-line bg-surface px-2 py-0.5 font-mono text-xs text-ink"
							>{{ agent.branch_name }}</code
						>
					</div>
					...
					<div
						...
						class="border-status-fix-fg/30 rounded-md border bg-status-fix-bg p-3 text-sm text-status-fix-fg"
					>
						{{ agent.error_message }}
					</div>
...
<script>
	...
	export default defineComponent({
		...
		watch: {
			agents: {
				handler(newAgents) {
					const hasActive = newAgents.some(
						(agent) =>
		...
		methods: {
			...
			async loadBranchesFromGitHub(search = null) {
				try {
					...
					this.branchesError =
						e.response?.data?.error || 'Failed to load branches from GitHub';
				} finally {

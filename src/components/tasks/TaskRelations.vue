<template>
	<div class="rounded-card border border-line bg-surface-sunken">
		...
		>
			...
			<button
				...
				class="inline-flex h-6 items-center justify-center gap-1 rounded-pill bg-brand px-2.5 text-xs font-medium text-white transition-colors hover:bg-brand-hover"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					class="h-3 w-3"
			...
			<div class="space-y-3">
				...
				>
					<div class="flex items-center gap-1">
						<component
							:is="getRelationIcon(type)"
							class="h-3 w-3 text-ink-subtle"
						/>
						<h4
					...
					<div class="space-y-0.5">
						<div
							...
							:key="relation.id"
							class="group flex items-center justify-between rounded-md border border-line bg-surface px-2 py-1.5 transition-colors hover:bg-surface-hover"
						>
							<div
							...
							<button
								...
								class="ml-1 inline-flex h-5 w-5 items-center justify-center rounded opacity-0 transition-colors hover:bg-status-fix-bg hover:text-status-fix-fg group-hover:opacity-100"
							>
								<svg
									xmlns="http://www.w3.org/2000/svg"
									class="h-3 w-3"
	...
	>
		<div
			class="fixed left-[50%] top-[50%] z-50 grid w-full max-w-md translate-x-[-50%] translate-y-[-50%] gap-4 rounded-card border border-line bg-surface p-5 text-ink shadow-tmgr-lg"
			@click.stop
		>
			<!-- Dialog Header -->
			...
			<div class="grid gap-3">
				...
				<div class="grid gap-1.5">
					...
					<div class="relative">
						<svg
							xmlns="http://www.w3.org/2000/svg"
							class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-subtle"
							viewBox="0 0 20 20"
							fill="currentColor"
						>
							<path
						...
						<input
							...
							type="text"
							placeholder="Search…"
							class="flex h-10 w-full rounded-md border border-line bg-surface-sunken px-3 pl-9 text-sm text-ink placeholder:text-ink-subtle focus-visible:border-line-strong focus-visible:outline-none"
							@input="
								handleTaskSearch(
					...
					<div
						v-if="taskSearchQuery && (searchResults.length > 0 || isSearching)"
						class="max-h-48 overflow-auto rounded-md border border-line bg-surface-sunken"
					>
						<div
							v-if="isSearching"
							class="py-3 text-center text-sm text-ink-subtle"
						>
							Searching…
						</div>
						<button
				...
				<div class="grid gap-1.5">
					...
					<div class="relative">
						<select
							...
							class="flex h-10 w-full appearance-none rounded-md border border-line bg-surface-sunken px-3 text-sm text-ink focus:border-line-strong focus:outline-none"
						>
				...
				<!-- Preview -->
				<div
					v-if="selectedTask && selectedRelationType"
				...
				<div
					v-if="error"
					class="border-status-fix-fg/30 rounded-md border bg-status-fix-bg px-3 py-2"
				>
			...
			<div class="flex justify-end gap-2">
				<button
					@click="cancelAdding"
					class="inline-flex h-9 items-center justify-center rounded-md border border-line bg-surface px-4 text-sm font-medium text-ink transition-colors hover:bg-surface-hover"
				>
					Cancel
				</button>
				<button
					...
					class="inline-flex h-9 items-center justify-center rounded-md bg-brand px-4 text-sm font-semibold text-white transition-colors hover:bg-brand-hover disabled:pointer-events-none disabled:opacity-50"
				>
...
<script setup lang="ts">
	...
	const selectedRelationType = computed(() => {
		...
		const type = relationTypes.value.find(
			(t) => t.id === Number(newRelation.value.typeId),
		);
			...
			'is dependency of': () =>
				h(
					...
					[
						...
						h('path', {
							'fill-rule': 'evenodd',
							d: 'M17 10a1 1 0 01-1 1H8a1 1 0 110-2h8a1 1 0 011 1z',
							'clip-rule': 'evenodd',
						}),
					],
				),
	...
	const handleAddRelation = async () => {
		...
		try {
			...
			error.value =
				err.response?.data?.message || 'Failed to add task relation';
	...
	const confirmRemoveRelation = async () => {
		...
		try {
			...
			error.value =
				err.response?.data?.message || 'Failed to remove task relation';

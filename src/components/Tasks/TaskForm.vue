<template>
	...
	<div ref="modal"
			 :class="`container mx-auto ${$color('blocks')} overflow-hidden ${isModal ? 'h-full' : ''} rounded-lg relative`">
		...
		<div :style="`height: ${middleBlockHeight}px`" class="text-center p-2.5">
			...
			<div v-if="!isCreatingTask" :key="checkpointUpdateKey" :class="`${$color('blocks')} rounded`">
					...
					<div v-for="(checkpoint, v) in form.checkpoints" :key="v" class="flex mb-1">
						<div class="w-full relative">
							...
							<span class="absolute right-0 top-0 mt-1.5 text-sm">
									...
									<span class="material-icons text-lg text-blue-700 checkpoint-delete"
												@click="changeCheckpointInputField(v)">edit</span>
		...
		<div ref="footer"
				 :class="`w-full p-5 shadow-top z-10 rounded-lg ${$color('blocks')} ${isModal ? 'absolute bottom-0' : ''}`">
			<task-actions

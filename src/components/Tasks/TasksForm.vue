		...
		<template #action>
			<div class="sm:flex items-between text-center">
				...
				<div v-if="!isCreatingTask" ref="editing_task_category">
					<a :class="$color('grayHover')" class="absolute left-0 top-0 text-gray-600 p-5 pt-4 md:p-0" href="#"
								...
								<form @submit.prevent="updateCategory">
									...
									<div>
										<label
											...
											for="">
											Estimated time
											<input-field v-model="form.approximately_time" :errors="errors.approximately_time"
																	 extra-class="bg-gray-400" placeholder="Enter approximately time"
									...
									<label class="tc-block text-gray-700 text-sm font-bold mb-5" for="settings">
										Settings
									</label>
									<div>
										<div v-for="(setting, index) in availableSettings" id="settings">
											...
											<div class="relative mb-4">
												<select v-if="!setting.show_custom_value_input"
												...
												<div v-if="setting.custom_value_available" class="b-switch-list">
													...
													>
														...
														>
															<div :class="$color('settingsTextColor')" class="b-switch-list__title">Set custom value
															</div>
														</div>
													</div>
												</div>
		...
		<template #body>
			<div :class="`${$color('blocks')} rounded-lg relative`">
					...
					<div class="lg:w-full h-full mt-5 p-5">
						<div :class="`bg-white pl-5 pr-5 h-full ${$color('blocks')}`">
							<div class="tc-block w-full float-left">
								<div class="mb-2">
									<input-field v-model="form.title" :errors="errors.title" placeholder="Enter task title" type="text" />
								</div>
							</div>
						</div>
					</div>
						...
						<div :class="`${$color('blocks')}  px-5 pb-1`">
							<div class="mb-2">
								...
								<p
									...
									<a class="text-blue-600 relative" href="#" style="top:2px"
				...
				<div class="w-full p-5">
					<h2 v-if="!isCreatingTask && form.checkpoints && form.checkpoints.length" class="text-2xl pt-5">
						Checkpoints
					</h2>
					<div v-if="!isCreatingTask" :key="checkpointUpdateKey" :class="`${$color('blocks')} rounded mt-5 p-5`">
						<div v-for="(checkpoint, v) in form.checkpoints" :key="v" class="mb-5">
							<div class="flex mb-2">
...
import Countdown from "src/components/Tasks/Countdown.vue";

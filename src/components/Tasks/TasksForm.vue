			...
			<div class="sm:flex items-between text-center">
				...
				<div v-if="!isCreatingTask" class="text-base sm:text-lg ml-auto">
					<button
						...
						:class="
							form.status_id !== status.id ? `opacity-25 hover:opacity-100` : ``
						"
		...
		<template #body>
			<div :class="`${$color('blocks')} rounded-lg relative`">
						...
						<div :class="`${$color('blocks')}  px-5 pb-1`">
							<div class="mb-2">
								...
								<input-field
									...
									type="textarea"
								/>
								<p v-else class="text-left whitespace-pre-line">
									<a
				...
				<div class="w-full p-5">
					<h2
						v-if="
							!isCreatingTask && form.checkpoints && form.checkpoints.length
						"
						class="text-2xl pt-5"
					>
						Checkpoints

...
<template>
    <div class="relative">
        ...
        <div v-if="isOpen" class="absolute right-0 z-10 mt-2 py-2 w-48 bg-white rounded-lg shadow-xl">
            <a v-for="(action, i) in actions" :key="i" href="#" class="block px-4 py-2 text-gray-800 hover:bg-indigo-500 hover:text-white" @click.prevent="action.click($event)">{{ action.label }}</a>
        </div>
    </div>
</template>

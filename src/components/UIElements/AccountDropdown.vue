<template>
	<div class="account relative select-none">
		...
		<div v-if="isOpenProfileDropdown" @click="isOpenProfileDropdown = false" tabindex="-1"
						class="fixed z-10 inset-0 h-full w-full bg-black opacity-50 cursor-default"></div>
		<div v-if="isOpenProfileDropdown" class="absolute right-0 mt-6 py-2 rounded-lg shadow-lg z-50" :class="`${ $color('bg') }`">
			<ul class="w-40 py-2 px-4">
				<li class="px-4 py-1 md:p-2 lg:px-4 hover:text-black">
					<router-link to="/settings" class="block" @click.native="isOpenProfileDropdown = false">
						Settings
					</router-link>
				</li>
				<li class="px-4 py-1 md:p-2 lg:px-4 hover:text-black">
					<a href="#" @click.prevent="logout" class="block">Logout</a>
				</li>
			</ul>
		</div>
	</div>

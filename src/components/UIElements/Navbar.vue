<template>
	<div class="ml-auto md:hidden absolute z-30 right-0 p-4">
		<div :class="{'active': menuIsActive}" class="nav-toggle-menu" @click="$store.getters.slideout.toggle()">
			<div :class="$color('burgerIcon')" class="bar1"></div>
			<div :class="$color('burgerIcon')" class="bar2"></div>
			<div :class="$color('burgerIcon')" class="bar3"></div>
		</div>
	</div>
	<nav :class="`${$color('menuBg')} shadow md:block tc-hidden`" role="navigation">
		<div class="container mx-auto p-4 flex flex-wrap items-center md:flex-no-wrap">
			<div :class="`w-full md:w-auto md:flex-grow md:flex md:items-center ${isHidden ? 'tc-hidden' : ''}`">
				<navbar-menu />
				<span :key="rerenderSwitcher" :class="`${$color('navTextUser')}-500`"

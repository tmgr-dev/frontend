...
<template>
	<div :class="stateClasses" :dir="dir" class="v-select">
		<slot name="header" v-bind="scope.header" />
		<div :id="`vs${uid}__combobox`" ref="toggle" :aria-expanded="dropdownOpen.toString()"
				 :aria-owns="`vs${uid}__listbox`"
				 aria-label="Search for option" class="vs__dropdown-toggle" role="combobox"
          ...
          <span :key="getOptionKey(option)" class="vs__selected">
            ...
            <button v-if="multiple" ref="deselectButtons" :aria-label="`Deselect ${getOptionLabel(option)}`"
										:disabled="disabled" :title="`Deselect ${getOptionLabel(option)}`"
										class="vs__deselect" type="button"
		...
		<transition :name="transition">
			<div>
				<ul v-if="dropdownOpen" :id="`vs${uid}__listbox`" :key="`vs${uid}__listbox`" ref="dropdownMenu"
						v-append-to-body class="vs__dropdown-menu" role="listbox" @mouseup="onMouseUp"
...
export default {
	...
	props: {
		 ...
		 * If you are using an array of objects, VueSelect will look for
		...
		/**
		 * Tells VueSelect what key to use when generating option

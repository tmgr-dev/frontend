<template>
	<div v-if="!isCreate" class="mt-6 border-t border-line pt-6">
		<h3 class="mb-4 text-base font-semibold text-ink">GitHub Integration</h3>
		...
		<div
			v-else-if="!status.has_installation"
			class="rounded-card border border-line bg-surface-sunken p-6 shadow-tmgr-xs"
		>
			<div class="mb-4 flex justify-center">
				<div
					class="flex h-12 w-12 items-center justify-center rounded-pill bg-surface text-ink-subtle"
				>
					<svg
						class="h-6 w-6"
						fill="none"
			...
			<p class="mx-auto mb-5 max-w-md text-center text-sm text-ink-muted">
				Link your GitHub repository to automatically track commits, branches,
				and pull requests
			</p>
			<button
		...
		<div v-else-if="!status.repository" class="space-y-4">
			<div
				class="border-status-done/40 rounded-card border bg-status-done-bg p-4"
			>
			...
			<div
				v-if="status.available_repositories.length === 0"
				class="border-status-fix/40 rounded-card border bg-status-fix-bg p-4"
			>
				<p class="text-sm text-status-fix-fg">
					No repositories available. Make sure the GitHub App has access to your
					repositories.
				</p>
			</div>
		...
		<div v-else class="space-y-4">
			...
			>
				<div class="flex items-center justify-between gap-3">
					<div>
						...
						<p
							v-if="status.repository.last_synced_at"
							class="text-status-done-fg/80 mt-1 text-xs"
						>
							Last synced: {{ formatDate(status.repository.last_synced_at) }}
						</p>
					</div>
					<svg
						class="h-5 w-5 shrink-0 text-status-done-fg"
						fill="none"
						stroke="currentColor"
						viewBox="0 0 24 24"
					>
			...
			<div class="flex flex-wrap gap-2">
				...
				<button
					...
					class="border-status-fix/40 inline-flex h-9 items-center justify-center rounded-pill border bg-status-fix-bg px-4 text-sm font-semibold text-status-fix-fg transition-colors hover:opacity-90 disabled:opacity-50"
				>
		...
		<div
			v-if="error"
			class="border-status-fix/40 mt-4 rounded-card border bg-status-fix-bg p-4"
		>
		...
		methods: {
			...
			async connectRepository() {
				...
				try {
					...
					this.error =
						e.response?.data?.error || 'Failed to connect repository';
				} finally {
			...
			async disconnectRepository() {
				...
				try {
					...
					this.error =
						e.response?.data?.error || 'Failed to disconnect repository';
				} finally {
			...
			async relinkRepository() {
				try {
					...
					this.error =
						e.response?.data?.error || 'Failed to relink task references';
				} finally {

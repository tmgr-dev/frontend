<template>
	<div v-if="!isCreate" class="mt-6 border-t border-line pt-6">
		<h3 class="mb-4 text-base font-semibold text-ink">Cursor AI Integration</h3>
		...
		>
			...
			<p class="mx-auto mb-5 max-w-md text-center text-sm text-ink-muted">
				Add your Cursor API key to enable AI-powered code generation for this
				category
			</p>
			...
			<div class="mt-4 space-y-3">
				<div>
					...
					<p class="mt-1.5 text-xs text-ink-muted">
						...
						<a
							...
							target="_blank"
							class="text-brand-fg hover:underline"
						>
							Cursor Dashboard
						</a>
					</p>
				...
				<div
					v-if="error"
					class="border-status-fix/40 rounded-card border bg-status-fix-bg p-3 text-sm text-status-fix-fg"
				>
					{{ error }}
				</div>
		...
		<div v-else class="space-y-4">
			<div class="border-brand/30 rounded-card border bg-brand-bg p-4">
				<div class="flex items-center justify-between gap-3">
					<div>
						<p class="text-sm font-semibold text-brand-fg">
							Cursor AI Agent Configured
						</p>
						<p class="text-brand-fg/80 mt-1 text-xs">
							Ready to assist with code generation for this category
						</p>
					</div>
					<svg
						class="h-5 w-5 shrink-0 text-brand-fg"
						fill="none"
						stroke="currentColor"
						viewBox="0 0 24 24"
					>
			...
			<div
				v-if="editing"
				class="mt-4 space-y-3 rounded-card border border-line bg-surface-sunken p-4"
			>
				<div>
					<label
						for="cursorApiKeyEdit"
						class="mb-1.5 block text-sm font-semibold text-ink"
					>
						New Cursor API Key
					</label>
	...
	export default defineComponent({
		...
		methods: {
			...
			async removeKey() {
				if (
					!confirm(
						'Are you sure you want to remove Cursor API key from this category?',
					)
				) {
					return;
				}

<template>
	<div
		:class="`relative rounded font-bold text-white shadow-md hover:rounded focus:outline-none ${buttonClass} w-full`"
	>
		<button
			:class="`h-full px-4`"
			type="button"
			@click.prevent="
				() => {
					actions[defaultActionIndex].action();
	...
	export default {
		...
		watch: {
			showDropdownButtons() {
				this.$nextTick(() => {
					this.dropdownOffset =
						this.direction === 'up' ? -this.$refs.dropdown.clientHeight : 0;
				});
			},
		},
		data() {

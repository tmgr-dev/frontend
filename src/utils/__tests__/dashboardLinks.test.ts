...
describe('dashboardStatisticTarget', () => {
	it('sends time tracking to the stats page', () => {
		expect(dashboardStatisticTarget({ view: 'time_tracking' })).toEqual({
			path: '/stats',
		});
	});

	it('uses the route daily routines actually live at', () => {
		expect(dashboardStatisticTarget({ view: 'daily_routine' })).toEqual({
			path: '/daily-routines',
		});
	});

...
describe('overtime', () => {
	...
	const fromSetting = {
		common_time: 5400,
		settings: [{ key: 'approximately_time', value: '3600' }],
	};
	...
	it('takes the estimate from the task, then from the category setting', () => {
		...
		expect(
			taskEstimateSeconds({
				settings: [{ key: 'approximately_time', pivot: { value: 600 } }],
			}),
		).toBe(600);
	...
	it('counts only time beyond a positive estimate', () => {
		...
		expect(pageOvertimeSeconds([over, under, fromSetting, noEstimate])).toBe(
			5400,
		);
	});
	...
	it('prefers the server total over the page sum', () => {
		expect(
			totalOvertimeSeconds({ total_overtime_seconds: '108000' }, [over]),
		).toBe(108000);

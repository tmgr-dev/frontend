...
const columns = (): PatchableColumn[] => [
	{
		status: { id: 1 },
		tasks: [
			{ id: 10, status_id: 1, title: 'a' },
			{ id: 11, status_id: 1, title: 'b' },
		],
	},
...
describe('upsertTaskInColumns', () => {
	it('inserts a new task at the top of its column', () => {
		const cols = columns();
		expect(
			upsertTaskInColumns(cols, { id: 12, status_id: 1, title: 'new' }),
		).toBe('inserted');
	...
	it('replaces a task in place when the status is unchanged, keeping fields the patch lacks', () => {
		const cols = columns();
		expect(
			upsertTaskInColumns(cols, { id: 11, status_id: '1', common_time: 5 }),
		).toBe('updated');
		expect(cols[0].tasks[1]).toEqual({
			id: 11,
			status_id: '1',
			title: 'b',
			common_time: 5,
	...
	it('drops a task whose new status has no column, and ignores unknown tasks for unknown columns', () => {
		const cols = columns();
		expect(upsertTaskInColumns(cols, { id: 20, status_id: 99 })).toBe(
			'removed',
		);
		expect(cols[1].tasks).toEqual([]);
		expect(upsertTaskInColumns(cols, { id: 30, status_id: 99 })).toBe(
			'ignored',
		);
	});
...
describe('upsertTaskInList', () => {
	...
	it('replaces a matching task, drops one that stopped matching', () => {
		...
		expect(
			upsertTaskInList(
				tasks,
				{ id: 2, status_id: 1, title: 'x' },
				{ accepts, firstPage: true },
			),
		).toBe('updated');

...
describe('timerStateDiffers', () => {
	it('sees a timer that was started in another tab', () => {
		expect(
			timerStateDiffers(
				{ start_time: null, common_time: 120 },
				{ start_time: 1757700000, common_time: 120 },
			),
		).toBe(true);
	});
	...
	it('sees a timer that was stopped in another tab', () => {
		expect(
			timerStateDiffers(
				{ start_time: 1757700000, common_time: 120 },
				{ start_time: null, common_time: 300 },
			),
		).toBe(true);
	});
	...
	it('compares numbers a string would hide', () => {
		expect(
			timerStateDiffers(
				...
				{ start_time: '1757700000' as unknown as number },
			),
		).toBe(false);
	});
});
...
describe('applyTimerState', () => {
	...
	it('clears the timer when the other tab stopped it, keeping the tracked total', () => {
		...
		expect(
			applyTimerState(form, { id: 5, start_time: null, common_time: 300 }),
		).toBe(true);
	...
	it('ignores an event about another task', () => {
		...
		expect(
			applyTimerState(form, { id: 6, start_time: 1757700000, common_time: 0 }),
		).toBe(false);
		expect(form.start_time).toBeNull();
	});

...
export const getActivityFeed = async (
	...
	try {
		...
		return createActionResult<PaginatedResponse<Activity>>(
			undefined,
			actionError,
		);
	}
...
export const refreshDashboardSection = async (
	...
	try {
		...
		return createActionResult(data.data, undefined, status, false, duration);
	} catch (error) {
...
export const refreshMultipleSections = async (
	...
	try {
		...
		return createActionResult(
			data,
			hasErrors
				? createActionError('Some sections failed to refresh', 'server', {
						errors,
						successful_sections: Object.keys(data),
				  })
				: undefined,
			200,
			false,
...
export const withRetry = async <T>(
	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		try {
			...
			lastError = createActionError('Retry operation failed', 'server', error);
		}
	}

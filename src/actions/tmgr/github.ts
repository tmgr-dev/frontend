...
export async function syncGitHubInstallations(): Promise<{
	message: string;
	count: number;
...
export async function getCategoryGitHubStatus(
	categoryId: number,
): Promise<GithubCategoryStatus> {
...
export async function disconnectCategoryRepository(
	categoryId: number,
): Promise<void> {
	await $axios.delete(`/categories/${categoryId}/github`);
}

export async function syncCategoryRepository(
	categoryId: number,
): Promise<{ message: string; last_synced_at: string }> {
...
export async function getTaskGitActivity(
	taskId: number,
): Promise<TaskGitActivity> {
	const response = await $axios.get(`/tasks/${taskId}/github`);
	return response.data;
}

export async function getTaskGitCommits(
	taskId: number,
...
export async function getTaskGitBranches(
	taskId: number,
...
export async function getTaskGitPullRequests(
	taskId: number,
